...
Спроектировать ERD логического уровня для учебного проекта **«Сервис бронирования отелей»**, отражающую основные сущности предметной области, их атрибуты и взаимосвязи.

---

## Описание

ERD-диаграмма логического уровня разработана на основе ранее выполненного анализа предметной области, бизнес-требований и BPMN-модели процесса бронирования отеля.

Диаграмма отражает структуру данных системы и описывает логические связи между основными сущностями, необходимыми для реализации процесса онлайн-бронирования гостиничных номеров.

В модели выделены ключевые сущности системы:
...
- Address (Адрес);
- Amenity (Удобство);
- RoomAmenity (Связующая сущность между номерами и удобствами);
- Review (Отзыв).
...
<p align="center">
    <img src="erd.png" alt="ERD логического уровня" width="1200">
</p>

<p align="center">
<b>Рисунок 1 — ERD логического уровня информационной системы бронирования отелей.</b>
</p>
...
Содержит сведения обо всех оформленных бронированиях.
...
Хранит информацию об отелях, доступных для бронирования.
...
Хранит сведения о произведенных платежах.
...
Содержит информацию о категориях гостиничных номеров.

---

## Address

Хранит адресную информацию, используемую отелями.
...
Является связующей сущностью между номерами и удобствами и реализует связь типа «многие ко многим».

---

## Review

Хранит отзывы клиентов об отелях.
...
|--------|-----------|----------|
| Client → Booking | Один ко многим | Один клиент может оформить несколько бронирований. |
| Hotel → Room | Один ко многим | Один отель содержит несколько гостиничных номеров. |
| Room → Booking | Один ко многим | Один номер может участвовать в различных бронированиях в разные даты. |
| Booking → Payment | Один к одному | Каждому бронированию соответствует один платеж. |
| Hotel → Address | Один к одному | Каждый отель имеет один адрес. |
| Room → RoomType | Многие к одному | Каждый номер относится к определенному типу номера. |
| Room → RoomAmenity | Один ко многим | Один номер может иметь несколько удобств. |
| Amenity → RoomAmenity | Один ко многим | Одно удобство может использоваться в различных номерах. |
| Client → Review | Один ко многим | Один клиент может оставить несколько отзывов. |
| Hotel → Review | Один ко многим | Один отель может иметь множество отзывов. |

---

# Итог

Разработанная ERD-диаграмма логического уровня отражает структуру данных информационной системы бронирования отелей и взаимосвязи между основными сущностями предметной области.

Полученная модель может быть использована в качестве основы для последующего проектирования физической модели базы данных и реализации системы бронирования.
****
